...
public class QuerySelectFieldListParser implements QueryParser {
    ...
    private AbstractAggregationBuilder parseStatsAggregation(SQLAggregateExpr aggExpr, String fieldName) {
        ...
        if (ElasticSqlMethodInvokeHelper.AGG_COUNT_METHOD.equalsIgnoreCase(methodName)) {
            return AggregationBuilders.sum(String.format("%s_%s", ElasticSqlMethodInvokeHelper.AGG_COUNT_METHOD, fieldName)).field(fieldName);
        }
